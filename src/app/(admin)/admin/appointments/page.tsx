...
import { getAppointmentsClient, updateAppointmentStatusClient } from "@/lib/queries/admin-client"
import {
  Calendar,
  Clock,
  PawPrint,
    ...
    return (
      <div className="w-full" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        >
          ...
          >
            <div className="h-6 w-48 rounded animate-pulse" style={{ background: 'var(--bg-tertiary)' }} />
            <div className="h-6 w-24 rounded animate-pulse" style={{ background: 'var(--bg-tertiary)' }} />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3" style={{ gap: '16px' }}>
            ...
            {filteredAppointments.length === 0 && (
              <div className="text-center" style={{ padding: '32px', color: 'var(--text-muted)' }}>
                <Calendar className="h-12 w-12 mx-auto opacity-50" style={{ marginBottom: '8px' }} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="flex items-start" style={{ gap: '12px' }}>
            <div
              ...
              style={{
                background: confirmAction?.type === 'cancel' ? 'var(--accent-red-bg)' : 'var(--accent-purple-bg)'
              }}
            >
              <AlertTriangle
                className="h-5 w-5"
                style={{
                  color: confirmAction?.type === 'cancel' ? 'var(--accent-red)' : 'var(--accent-purple)'
                }}
              />
            </div>
          ...
          <div className="grid grid-cols-2" style={{ gap: '12px', marginTop: '8px' }}>
            <Button
              variant="secondary"
              onClick={() => {
                setIsConfirmModalOpen(false)
            ...
            <Button
              ...
              style={{ height: '52px' }}
            >
              {isUpdating ? 'Processando...' : getConfirmModalContent().buttonText}
            </Button>
          </div>
        </div>
      </Modal>
        ...
        {selectedAppointment && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div className="flex items-center justify-between">
              <Badge variant={statusVariants[selectedAppointment.status] || 'info'} className="text-sm">
              ...
              <span className="text-xl font-bold" style={{ color: 'var(--accent-green)' }}>
                {formatCurrency(selectedAppointment.price || selectedAppointment.service?.base_price || 0)}
              </span>
            </div>
              ...
              <div className="flex items-center" style={{ gap: '12px', marginBottom: '12px' }}>
                ...
                <div>
                  ...
                  <p className="font-semibold" style={{ color: 'var(--text-primary)' }}>
                    {selectedAppointment.service?.name} ({selectedAppointment.service?.duration_min || 60} min)
                  </p>
                </div>
              </div>
              <div className="flex items-center" style={{ gap: '12px' }}>
                ...
                <div>
                  ...
                  <p className="font-semibold" style={{ color: 'var(--text-primary)' }}>
                    {formatDate(selectedAppointment.scheduled_date)} às {selectedAppointment.scheduled_time?.slice(0, 5)}
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              <h3 className="font-semibold flex items-center" style={{ gap: '8px', marginBottom: '12px', color: 'var(--text-primary)' }}>
                <PawPrint className="h-5 w-5" style={{ color: 'var(--accent-pink)' }} />
                Pet
              </h3>
              <div className="flex" style={{ gap: '16px' }}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center" style={{ gap: '8px', marginTop: '4px' }}>
                    <span
                    ...
                    >
                      {speciesLabels[selectedAppointment.pet?.species || ''] || selectedAppointment.pet?.species}
                    </span>
                    ...
                    {selectedAppointment.pet?.size && (
                      ...
                      >
                        ...
                        {sizeLabels[selectedAppointment.pet.size] || selectedAppointment.pet.size}
                      </span>
                    )}
                  </div>
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                {formatFullAddress(selectedAppointment.user) && (
                  <div>
                    <div className="flex items-start" style={{ gap: '8px', color: 'var(--text-muted)' }}>
                      <MapPin className="h-4 w-4 flex-shrink-0 mt-0.5" />
                      <div className="text-sm">
                        <p>{formatFullAddress(selectedAppointment.user)?.line1}</p>
                        <p>{formatFullAddress(selectedAppointment.user)?.line2}</p>
                        {formatFullAddress(selectedAppointment.user)?.zip && (
                          <p>CEP: {formatFullAddress(selectedAppointment.user)?.zip}</p>
                        )}
                      </div>
                    </div>
            ...
            {selectedAppointment.notes && (
              ...
              >
                <h3 className="font-semibold flex items-center" style={{ gap: '8px', marginBottom: '8px', color: 'var(--text-primary)' }}>
                  <FileText className="h-5 w-5" style={{ color: 'var(--accent-yellow)' }} />
            ...
            {(selectedAppointment.status === "completed" || selectedAppointment.status === "cancelled") && (
              <div style={{ marginTop: '8px' }}>
                <Button variant="secondary" onClick={() => setIsDetailModalOpen(false)} className="w-full" style={{ height: '52px' }}>
                  Fechar
                </Button>
              </div>
            )}
